...
OUTPUT_STEPS, EPOCHS, BATCH_SIZE, LEARNING_RATE = 5, 25, 64, 0.001
...
if __name__ == "__main__":
    ...
    for i in range(OUTPUT_STEPS):
        ...
        y_dummy[:, 0] = y[:, i]
        y_scaled[:, i] = scaler.transform(y_dummy)[:, 0]
    ...
    SCALER_FILE = os.path.join(MODEL_SAVE_PATH, "scaler_regression.pkl")
    torch.save({"model_state": model.state_dict()}, MODEL_FILE)
    print(f"(+) Regression model saved to {MODEL_FILE}")
    joblib.dump(scaler, SCALER_FILE)
    print(f"(+) Regression scaler saved to {SCALER_FILE}")